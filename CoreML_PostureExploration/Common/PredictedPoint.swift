...
struct PredictedPoint:Identifiable {
    var id = UUID()
    ...
    func getName(selfArray:[PredictedPoint?]) -> String? {
        if let i = selfArray.toUnwrapped().firstIndex(where: {$0.id == self.id}){
            return PoseEstimationForMobileConstant.pointLabels[i]
        } else {
            return nil
        }
    }
...
extension Array where Element == PredictedPoint? {
    func toUnwrapped() -> [PredictedPoint]{
        var returnArray:[PredictedPoint] = []
        for elem in self {
            if let elem = elem{
