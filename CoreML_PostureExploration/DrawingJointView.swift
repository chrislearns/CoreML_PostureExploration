...
//  udemy_hotdogApp
//
//  Created by Christopher Guirguis on 1/31/21.
//
...
struct DrawingJointView:UIViewRepresentable {
    typealias UIViewType = DrawingJointUIView
    @ObservedObject var jointModel:JointModel
    
    
    func makeUIView(context: Context) -> DrawingJointUIView {
        let view = DrawingJointUIView(frame: UIScreen.main.bounds)
        view.bodyPoints = jointModel.points
        return view
    }
    
    func updateUIView(_ uiView: UIViewType, context: Context) {
        
        uiView.bodyPoints = jointModel.points
    }
}

class DrawingJointUIView: UIView {
    ...
    public var jointModel:JointModel = JointModel(){
        didSet {
            self.setNeedsDisplay()
            self.drawKeypoints(with: self.bodyPoints)
        }
    }
        ...
        if let ctx = UIGraphicsGetCurrentContext() {
            ...
            if PoseEstimationForMobileConstant.pointLabels.count == bodyPoints.count {
                let _ = PoseEstimationForMobileConstant.connectedPointIndexPairs.map { pIndex1, pIndex2 in
                    if let bp1 = self.bodyPoints[pIndex1], bp1.maxConfidence > DrawingJointUIView.threshold,
                       let bp2 = self.bodyPoints[pIndex2], bp2.maxConfidence > DrawingJointUIView.threshold {
                        let p1 = bp1.maxPoint
                        let p2 = bp2.maxPoint
